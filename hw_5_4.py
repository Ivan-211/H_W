...
class House :
    ...
    def __new__(cls, *args): #name,*args
        __insistance = super().__new__(cls)
        cls.houses_history.append(args[0]) #name
        return __insistance
    ...
    def __eq__(self,others):
         if isinstance(self,House) and isinstance(others,House):
    ...
    def __add__(self, value):
        if isinstance(self.number_of_floors,int) and isinstance(value,int):
            ...
            return f'Я немогу слoжить {type(self.number_of_floors)} и {type(value)}'
